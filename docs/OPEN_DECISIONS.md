...
**Historical Implementation Record:** See [IMPLEMENTATION_RECORD.md](IMPLEMENTATION_RECORD.md) for complete Part A & Part B implementations (B1 scaling, intra-surface single-linkage clustering, concurrency locking, EVT hurdles, Lo SE Z-tests, Invariant 8, and statistical hardening).
...
| **B5** | Representative Election Granularity | **RESOLVED & IMPLEMENTED** | Election groups on the surface `structure` tuple, not `structural_hash`: siblings are never correlation-checked against each other, so a finer key would surface never-compared near-duplicates. See §4. |
