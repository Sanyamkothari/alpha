    ...
    while True:
        ...
        if simulate:
            ...
            if ids:
                ...
                # `simulated` is the count run_batch actually landed. Deriving it
                # from len(ids) instead would over-count: ids can name alphas the
                # batch never built a job for (deleted between the two queries).
